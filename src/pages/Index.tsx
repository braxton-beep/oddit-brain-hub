  ...
  const { data: phaseCounts } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase.from("report_drafts").select("*", { count: "exact", head: true }),
        supabase.from("cro_audits").select("*", { count: "exact", head: true }).not("recommendations", "is", null),
        supabase.from("wireframe_briefs").select("*", { count: "exact", head: true }),
        supabase.from("setup_runs").select("*", { count: "exact", head: true }).eq("status", "done"),
        supabase.from("figma_files").select("*", { count: "exact", head: true }),
        supabase.from("client_brand_assets").select("*", { count: "exact", head: true }),
        supabase.from("pipeline_projects").select("*", { count: "exact", head: true }),
        supabase.from("generated_sections").select("*", { count: "exact", head: true }),
      ...
      return {
        ...
        setups: setups ?? 0,
        figma: figma ?? 0,
        brandAssets: brandAssets ?? 0,
        pipeline: pipeline ?? 0,
        shopifyCode: shopifyCode ?? 0,
